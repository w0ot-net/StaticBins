...
- Bootstrap commit `bb8ede2` added the architecture-owned builder and the
  bounded publication-workflow case without exposing ARMv7 to recipes.
  Adoption commit `3f35240` added the verified public digest, dispatcher and
  validator allowlists, focused tests, and documentation, then fast-forwarded
  directly to `main`.
- Alpine 3.24.1's exact `linux/arm/v7` child manifest was reconfirmed as
  `sha256:48bf253520b161ff0f7bd9c6b2aded4126fa8ee2bc29386580b2a8a0322a1742`.
  Its repositories resolved the only direct package rows as
  `build-base=0.5-r4` and `file=5.47-r2`.
- `./builders/armv7/build.sh` built one local candidate and passed runtime
  `armv7l`, exact package/command/archive checks, static C and C++ execution,
  ELF32, little-endian ARM, hard-float EABI, `ET_EXEC`, no interpreter, no
  `DT_NEEDED`, OCI `arm`/`v7`, and repository-label checks.
- Publication run `31198775381` ran on the ARM64 hosted runner, registered only
  the pinned `arm` binfmt handler, observed `armv7l`, and published both ARMv7
  tags at immutable digest
  `sha256:d89be01b0c8eccf788f09e806d750a9284bf387da868133e8d6168073fe069d3`.
  Anonymous inspection found the expected `linux/arm/v7` payload and attached
  SPDX SBOM and SLSA provenance statements; both tags resolved to that digest.
- A fresh anonymous digest-pinned container repeated the package, runtime,
  archive, static C/C++, ELF, ABI, and execution probes successfully. Workflow
  YAML, every embedded Bash block, shell syntax, ASCII checks, and
  `git diff --check` also passed.
- Catalog validation and all 20 unit tests passed; `./build.sh list` remained
  exactly the existing two rows while ARMv7 fixtures were accepted and `arm`,
  `armhf`, and `armeb` were rejected. Main runs `31199566021` and `31199569406`
  passed `recipe-validation`, the selected exact tcpdump rebuild/attestation,
  and `artifact-assurance`.
- No recipe, catalog row, artifact directory, checksum, trust claim, existing
  builder, VM, self-hosted runner, cross-toolchain, or utility image was added
  or changed.
