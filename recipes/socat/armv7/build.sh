...
readonly OUTPUT_FILE="${OUTPUT_DIR}/socat"
...
echo "Building socat ${SOURCE_VERSION} for ARMv7 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
docker run --rm \
    --platform "${PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/socat,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-test.sh,dst=/smoke-test,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /socat "${SOURCE_VERSION}"
...
if [[ "${installed_sha256}" != "${candidate_sha256}" ]]; then
    echo "error: installed socat does not match the validated candidate" >&2
    exit 1
fi
