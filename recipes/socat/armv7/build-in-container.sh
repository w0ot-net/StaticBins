...
file /out/socat
if ! file /out/socat | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
    echo "error: socat is not an ELF32 little-endian ARM executable" >&2
    exit 1
fi
if ! readelf -h /out/socat | grep -Eq 'Class:[[:space:]]+ELF32'; then
    echo "error: socat is not ELF32" >&2
    exit 1
fi
if ! readelf -h /out/socat | grep -Eq 'Data:[[:space:]]+2.s complement, little endian'; then
    echo "error: socat is not little-endian" >&2
    exit 1
fi
...
if ! readelf -h /out/socat | grep -Eq 'Flags:.*hard-float ABI'; then
    echo "error: socat does not declare the ARM hard-float ABI" >&2
    exit 1
fi
