...
build_cflags="-O2 -pipe -march=i686 -msse2 -mfpmath=sse"
...
for required_input in "${archive_inventory}" "${source_patch}"; do
...
if ! readelf -hW /out/ltrace | grep -Eq 'Class:[[:space:]]+ELF32' ||
    ! readelf -hW /out/ltrace | grep -Eq \
        'Data:[[:space:]]+2.s complement, little endian'; then
    echo "error: ltrace is not little-endian ELF32 x86" >&2
    exit 1
fi
if ! readelf -hW /out/ltrace | grep -Eq \
