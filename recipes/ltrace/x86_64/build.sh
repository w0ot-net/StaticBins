...
echo "Building ltrace ${SOURCE_VERSION} for x86-64 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
candidate="${temporary_dir}/ltrace"
...
if ! readelf -hW "${smoke_target}" | grep -Eq \
    'Machine:[[:space:]]+Advanced Micro Devices X86-64'; then
    echo "error: smoke target is not an x86-64 executable" >&2
    exit 1
fi
if ! readelf -lW "${smoke_target}" | grep -q \
    'Requesting program interpreter: /lib/ld-musl-x86_64.so.1'; then
    echo "error: smoke target is not dynamically linked against x86-64 musl" >&2
    exit 1
fi
docker run --rm \
    --platform "${PLATFORM}" \
    ...
    "${BUILDER_IMAGE}" \
    /smoke-test /ltrace /smoke-target "${SOURCE_VERSION}"
