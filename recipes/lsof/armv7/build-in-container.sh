...
file /out/lsof
if ! file /out/lsof | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
    echo "error: lsof is not an ELF32 little-endian ARM executable" >&2
    exit 1
fi
...
if ! readelf -h /out/lsof | grep -Eq 'Flags:.*hard-float ABI'; then
    echo "error: lsof does not use the ARM hard-float ABI" >&2
    exit 1
fi
