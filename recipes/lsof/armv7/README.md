...
ARMv7 builder in `builders/armv7/environment.lock`, registering the pinned ARM
binfmt helper only when native container execution is unavailable, and replaces
`artifacts/armv7/lsof` only after validation.
...
`ET_EXEC`, exact version and feature output, complete final-link evidence, and
successful procfs discovery of a uniquely named file held open by a controlled
process.
