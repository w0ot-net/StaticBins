...
readonly OUTPUT_FILE="${OUTPUT_DIR}/lsof"
...
echo "Building lsof ${SOURCE_VERSION} for ARMv7 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
validate_elf() {
    ...
    if ! readelf -h "${binary}" | grep -Eq 'Flags:.*hard-float ABI'; then
        echo "error: output does not use the ARM hard-float ABI" >&2
        exit 1
    fi
...
docker run --rm \
    --platform "${PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/lsof,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-test.sh,dst=/smoke-test,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /lsof "${SOURCE_VERSION}"
...
if [[ "${installed_sha256}" != "${candidate_sha256}" ]]; then
    echo "error: installed lsof does not match the validated candidate" >&2
    exit 1
fi
