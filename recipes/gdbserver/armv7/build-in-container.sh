...
file /out/gdbserver
if ! file /out/gdbserver | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
    echo "error: GDBserver is not an ELF32 little-endian ARM executable" >&2
    exit 1
fi
if ! readelf -h /out/gdbserver | grep -Eq 'Class:[[:space:]]+ELF32'; then
    echo "error: GDBserver is not ELF32" >&2
    exit 1
fi
if ! readelf -h /out/gdbserver | grep -Eq 'Data:[[:space:]]+2.s complement, little endian'; then
    echo "error: GDBserver is not little-endian" >&2
    exit 1
fi
...
if ! readelf -h /out/gdbserver | grep -Eq 'Flags:.*hard-float ABI'; then
    echo "error: GDBserver does not declare the ARM hard-float ABI" >&2
    exit 1
fi
