...
echo "Building GDBserver ${SOURCE_VERSION} for x86-64 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
docker run --rm \
    ...
    --mount "type=bind,src=${SCRIPT_DIR}/rsp-smoke.c,dst=/rsp-smoke.c,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-target.c,dst=/smoke-target.c,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /gdbserver "${SOURCE_VERSION}" /rsp-smoke.c /smoke-target.c
