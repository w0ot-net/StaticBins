# Static GNU readelf 2.47 for x86

From the repository root, run:

```sh
./build.sh readelf x86
```
...
immutable x86 builder lock, then replaces `artifacts/x86/readelf` only
after validation.
...
program headers, sections, symbols, and DWARF data in a controlled
i686-compatible executable.
