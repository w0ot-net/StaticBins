# Static GNU Netcat 0.7.1 for AArch64
...
`readelf`, and `sha256sum`. It consumes the immutable AArch64 builder
in `builders/aarch64/environment.lock` and replaces
`artifacts/aarch64/nc` only after complete validation.
...
The build disables NLS, uses the documented aarch64 compiler baseline, and
