# Static GNU Netcat 0.7.1 for 32-bit x86
...
`readelf`, and `sha256sum`. It consumes the immutable 32-bit x86 builder
in `builders/x86/environment.lock` and replaces
`artifacts/x86/nc` only after complete validation.
...
The build disables NLS, uses the documented x86 compiler baseline, and selects
the locked toolchain's stripped static PIE `ET_DYN` profile without an
