...
configure_triplet="i686-pc-linux-gnu"
...
if ! CFLAGS="-O2 -pipe -march=i686 -msse2 -mfpmath=sse"     LDFLAGS="-static -Wl,-Map,/out/nc-link.map"     ./configure         --build="${configure_triplet}"         --host="${configure_triplet}"         --disable-dependency-tracking         --disable-nls > /out/nc-configure.log 2>&1; then
