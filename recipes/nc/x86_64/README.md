# Static GNU Netcat 0.7.1 for x86-64
...
`readelf`, and `sha256sum`. It consumes the immutable x86-64 builder
in `builders/x86_64/environment.lock` and replaces
`artifacts/x86_64/nc` only after complete validation.
...
The build disables NLS, uses the documented x86_64 compiler baseline, and
