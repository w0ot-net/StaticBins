...
readonly EXPECTED_OPTIONAL_FEATURES="no-m32-mpers no-mx32-mpers"
...
if ! docker pull --platform "${PLATFORM}" "${BUILDER_IMAGE}"; then
    ...
    echo "Publish and lock a replacement with ./builders/x86_64/build.sh and .github/workflows/publish-builder.yml." >&2
    exit 1
fi
...
echo "Building strace ${SOURCE_VERSION} for x86-64 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
docker run --rm \
    ...
    "${BUILDER_IMAGE}" \
    /smoke-test /strace "${SOURCE_VERSION}" "${EXPECTED_OPTIONAL_FEATURES}"
