...
for command_name in cpio curl docker file gzip qemu-system-i386 readelf sha256sum timeout; do
...
if [[ ! -f "${kernel}" ]]; then
    ...
    echo "Downloading the pinned x86 smoke-test kernel..."
...
echo "Building strace ${SOURCE_VERSION} for x86 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
