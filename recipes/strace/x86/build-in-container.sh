...
mkdir -p "${build_dir}"
cd "${build_dir}"
if ! CFLAGS="-O2 -pipe -march=i686 -msse2 -mfpmath=sse" \
...
if ! file /out/strace | grep -Eq 'ELF 32-bit LSB executable, Intel (i386|80386)'; then
    echo "error: strace is not a 32-bit little-endian x86 executable" >&2
    exit 1
fi
