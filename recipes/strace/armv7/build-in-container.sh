...
source_lock="/usr/local/share/static_bins/strace/source.lock"
source_input_dir="/usr/local/share/static_bins/strace/sources"
license_dir="/usr/local/share/licenses/strace"
archive_inventory="${license_dir}/archive-inventory.tsv"
target_triplet="armv7-alpine-linux-musleabihf"
expected_machine="ARM"
expected_optional_features="(none)"

if [ ! -r "${source_lock}" ]; then
    echo "error: missing strace source lock: ${source_lock}" >&2
    exit 1
fi
...
source_dir="/build/strace-${SOURCE_VERSION}"
build_dir="/build/strace-build"

for command_name in cc cp file make readelf sha256sum strip tar; do
...
if [ ! -f "${source_input}" ]; then
    echo "error: missing tracked strace source archive: ${source_input}" >&2
    exit 1
fi
...
if ! CFLAGS="-O2 -pipe" \
    ...
    "${source_dir}/configure" \
        ...
        --without-libunwind \
        --without-libiberty \
        --without-libselinux > /out/strace-configure.log 2>&1; then
    cat /out/strace-configure.log >&2
    exit 1
fi
tail -n 3 /out/strace-configure.log

if ! make -C src -j"${BUILD_JOBS}" V=1 \
    strace_LDFLAGS="-Wl,-Map,/out/strace-link.map" \
    all > /out/strace-link.log 2>&1; then
    cat /out/strace-link.log >&2
    exit 1
fi
tail -n 2 /out/strace-link.log
...
' /out/strace-link.map | sort -u > /out/linked-archives.raw
...
while IFS= read -r linked_archive; do
    case "${linked_archive}" in
        ...
        *) archive_path="${build_dir}/src/${linked_archive}" ;;
    esac
    archive_path="$(readlink -f "${archive_path}")"

    case "${archive_path}" in
        "${build_dir}"/* | "${source_dir}"/*)
            printf 'strace-source\t%s\t%s\t%s\n' \
...
if [ "${internal_archive_count}" -eq 0 ]; then
    echo "error: final link map contained no strace-source archive" >&2
    inventory_errors=$((inventory_errors + 1))
fi
...
install -m 0755 src/strace /out/strace
strip /out/strace

file /out/strace
if ! file /out/strace | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
    echo "error: strace is not an ELF32 little-endian ARM executable" >&2
    exit 1
fi
if ! readelf -h /out/strace | grep -Eq 'Class:[[:space:]]+ELF32'; then
    echo "error: strace is not ELF32" >&2
    exit 1
fi
if ! readelf -h /out/strace | grep -Eq 'Data:[[:space:]]+2.s complement, little endian'; then
    echo "error: strace is not little-endian" >&2
    exit 1
fi
if ! readelf -h /out/strace | grep -Eq "Machine:[[:space:]]+${expected_machine}"; then
    echo "error: strace has the wrong ELF machine" >&2
    readelf -h /out/strace >&2
    exit 1
fi
if ! readelf -h /out/strace | grep -Eq 'Flags:.*hard-float ABI'; then
    echo "error: strace does not declare the ARM hard-float ABI" >&2
    exit 1
fi
if ! readelf -h /out/strace | grep -Eq 'Type:[[:space:]]+EXEC'; then
    echo "error: strace is not an ELF ET_EXEC executable" >&2
    exit 1
fi
if readelf -l /out/strace | grep -q 'Requesting program interpreter'; then
    echo "error: strace has a dynamic program interpreter" >&2
    exit 1
fi
if readelf -d /out/strace 2>/dev/null | grep -q '(NEEDED)'; then
    echo "error: strace has dynamic library dependencies" >&2
    readelf -d /out/strace | grep '(NEEDED)' >&2
    exit 1
fi
if readelf -S /out/strace | grep -Eq '[.]debug|[.]symtab'; then
    echo "error: strace retains debug or full symbol-table sections" >&2
    exit 1
fi

version_output="$(/out/strace --version 2>&1)"
printf '%s\n' "${version_output}"
if ! printf '%s\n' "${version_output}" | grep -Fqx \
    "strace -- version ${SOURCE_VERSION}"; then
