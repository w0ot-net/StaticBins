...
readonly OUTPUT_FILE="${OUTPUT_DIR}/strace"
...
readonly EXPECTED_OPTIONAL_FEATURES="(none)"
...
for command_name in cpio curl docker file gzip qemu-system-arm readelf sha256sum timeout; do
...
if [[ ! -f "${kernel}" ]]; then
    ...
    echo "Downloading the pinned ARMv7 smoke-test kernel..."
...
echo "Building strace ${SOURCE_VERSION} for ARMv7 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
candidate="${temporary_dir}/strace"
vm_smoke="${temporary_dir}/vm-smoke"
smoke_target="${temporary_dir}/smoke-target"
validate_elf() {
    local binary="$1"
    file "${binary}"
    if ! file "${binary}" | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
        echo "error: output is not an ELF32 little-endian ARM executable" >&2
        exit 1
    fi
    ...
    if ! readelf -h "${binary}" | grep -Eq 'Machine:[[:space:]]+ARM'; then
        echo "error: output is not an ARM executable" >&2
        exit 1
    fi
    if ! readelf -h "${binary}" | grep -Eq 'Flags:.*hard-float ABI'; then
        echo "error: output does not declare the ARM hard-float ABI" >&2
        exit 1
    fi
...
validate_elf "${vm_smoke}"
validate_elf "${smoke_target}"
version_output="$(docker run --rm \
    --platform "${PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/strace,readonly" \
    "${BUILDER_IMAGE}" \
    /strace --version)"
printf '%s\n' "${version_output}"
...
if [[ "${installed_sha256}" != "${candidate_sha256}" ]]; then
    echo "error: installed strace does not match the validated candidate" >&2
    exit 1
fi
