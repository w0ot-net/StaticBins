...
echo "Building GDB ${SOURCE_VERSION} for ARMv7 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
    "${build_args[@]}" \
...
validate_smoke_target() {
    ...
    file "${binary}"
    if ! file "${binary}" | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
        echo "error: smoke target is not an ELF32 little-endian ARM executable" >&2
        exit 1
    fi
    if ! readelf -h "${binary}" | grep -Eq 'Class:[[:space:]]+ELF32'; then
        echo "error: smoke target is not ELF32" >&2
        exit 1
    fi
    if ! readelf -h "${binary}" | grep -Eq 'Data:[[:space:]]+2.s complement, little endian'; then
    ...
    if ! readelf -h "${binary}" | grep -Eq 'Machine:[[:space:]]+ARM'; then
        echo "error: smoke target is not an ARM executable" >&2
        exit 1
    fi
    if ! readelf -h "${binary}" | grep -Eq 'Flags:.*hard-float ABI'; then
        echo "error: smoke target does not declare the ARM hard-float ABI" >&2
        exit 1
    fi
    if readelf -l "${binary}" | grep -q 'Requesting program interpreter'; then
