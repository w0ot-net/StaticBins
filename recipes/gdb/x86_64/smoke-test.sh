...
if ! printf '%s\n' "${inspection_output}" | grep -Fq \
    'currently "i386:x86-64"'; then
    echo "error: GDB did not report an x86-64 target architecture" >&2
    exit 1
fi
if ! printf '%s\n' "${inspection_output}" | grep -Fq \
...
echo "validated x86-64 BFD loading, architecture, symbol, and data inspection"
