...
if ! file /out/gdb | grep -Eq 'ELF 64-bit LSB executable, x86-64'; then
    echo "error: GDB is not an ELF64 little-endian x86-64 executable" >&2
    exit 1
fi
if ! readelf -h /out/gdb | grep -Eq 'Class:[[:space:]]+ELF64'; then
    echo "error: GDB is not ELF64" >&2
    exit 1
fi
...
if ! readelf -h /out/gdb | grep -Eq 'Machine:[[:space:]]+Advanced Micro Devices X86-64'; then
    echo "error: GDB is not an x86-64 executable" >&2
    readelf -h /out/gdb >&2
    exit 1
fi
