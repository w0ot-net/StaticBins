...
echo "Building GDB ${SOURCE_VERSION} for x86 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
    "${build_args[@]}" \
...
validate_smoke_target() {
    local binary="$1"
    if ! file "${binary}" | grep -Eq 'ELF 32-bit LSB executable, Intel (i386|80386)'; then
        echo "error: smoke target is not a 32-bit little-endian x86 executable" >&2
        exit 1
    fi
    if ! readelf -h "${binary}" | grep -Eq 'Class:[[:space:]]+ELF32'; then
    ...
    if ! readelf -h "${binary}" | grep -Eq 'Machine:[[:space:]]+Intel 80386'; then
        echo "error: smoke target is not an Intel 80386 executable" >&2
        exit 1
    fi
    if readelf -l "${binary}" | grep -q 'Requesting program interpreter'; then
