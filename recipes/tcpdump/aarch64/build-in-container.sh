...
for archive_path in \
    ...
    /usr/lib/gcc/aarch64-alpine-linux-musl/15.2.0/libgcc_eh.a; do
...
if ! file /out/tcpdump | grep -Eq 'ELF 64-bit LSB executable, ARM aarch64'; then
    echo "error: tcpdump is not an ELF64 little-endian AArch64 executable" >&2
    exit 1
fi
if ! readelf -h /out/tcpdump | grep -Eq 'Class:[[:space:]]+ELF64'; then
    echo "error: tcpdump is not ELF64" >&2
    exit 1
fi
...
if ! readelf -h /out/tcpdump | grep -Eq 'Machine:[[:space:]]+AArch64'; then
    echo "error: tcpdump is not an AArch64 executable" >&2
    exit 1
fi
