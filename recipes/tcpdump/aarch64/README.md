# AArch64 tcpdump recipe

This recipe builds tcpdump 4.99.4 with libpcap 1.10.4 as a stripped static
AArch64 executable. From the repository root, use the stable dispatcher or the
direct recipe command:

```sh
./build.sh tcpdump aarch64
./recipes/tcpdump/aarch64/build.sh
```
...
exact builder digest in `builders/aarch64/environment.lock` and both tracked
archives under `sources/`, validates a temporary candidate, and writes
`artifacts/aarch64/tcpdump` only after every check passes. On non-AArch64 Linux
hosts it may register the pinned
QEMU `binfmt_misc` helper with `--privileged` when AArch64 container support is
absent. Set `BUILD_JOBS` to tune compilation parallelism:

```sh
BUILD_JOBS=4 ./build.sh tcpdump aarch64
```
