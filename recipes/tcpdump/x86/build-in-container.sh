...
for archive_path in \
    ...
    /usr/lib/gcc/i586-alpine-linux-musl/15.2.0/libgcc.a \
    /usr/lib/gcc/i586-alpine-linux-musl/15.2.0/libgcc_eh.a; do
...
strip /out/tcpdump
if ! file /out/tcpdump | grep -Eq 'ELF 32-bit LSB executable, Intel (i386|80386)'; then
    echo "error: tcpdump is not a 32-bit little-endian x86 executable" >&2
    exit 1
fi
...
if ! readelf -h /out/tcpdump | grep -Eq 'Machine:[[:space:]]+Intel 80386'; then
    echo "error: tcpdump is not an Intel 80386 executable" >&2
    exit 1
fi
