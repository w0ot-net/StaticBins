...
for archive_path in \
    ...
    /usr/lib/gcc/armv7-alpine-linux-musleabihf/15.2.0/libgcc.a \
    /usr/lib/gcc/armv7-alpine-linux-musleabihf/15.2.0/libgcc_eh.a; do
...
if ! file /out/tcpdump | grep -Eq 'ELF 32-bit LSB executable, ARM'; then
    echo "error: tcpdump is not an ELF32 little-endian ARM executable" >&2
    exit 1
fi
if ! readelf -h /out/tcpdump | grep -Eq 'Class:[[:space:]]+ELF32'; then
    echo "error: tcpdump is not ELF32" >&2
    exit 1
fi
if ! readelf -h /out/tcpdump | grep -Eq 'Data:[[:space:]]+2.s complement, little endian'; then
    echo "error: tcpdump is not little-endian" >&2
    exit 1
fi
...
if ! readelf -h /out/tcpdump | grep -Eq 'Machine:[[:space:]]+ARM'; then
    echo "error: tcpdump is not an ARM executable" >&2
    exit 1
fi
if ! readelf -h /out/tcpdump | grep -Eq 'Flags:.*hard-float ABI'; then
    echo "error: tcpdump does not use the ARM hard-float ABI" >&2
    exit 1
fi
