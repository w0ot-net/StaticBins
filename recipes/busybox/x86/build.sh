...
readonly OUTPUT_FILE="${OUTPUT_DIR}/busybox"
...
if [[ "${SOURCE_AUTHENTICATION}" != checksum-only ]]; then
    echo "error: unexpected BusyBox source authentication mode: ${SOURCE_AUTHENTICATION}" >&2
    exit 1
fi

source_archive="${SCRIPT_DIR}/sources/${SOURCE_ARCHIVE}"
printf '%s  %s\n' "${SOURCE_SHA256}" "${source_archive}" | sha256sum -c -
echo "source authentication: checksum-only (upstream signing key not adopted)"

echo "Checking 32-bit x86 container support..."
...
echo "Building BusyBox ${SOURCE_VERSION} for 32-bit x86 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
candidate="${temporary_dir}/busybox"
"${SCRIPT_DIR}/validate-elf.sh" "${candidate}"
docker run --rm \
    --platform "${PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/busybox,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/expected-applets.txt,dst=/expected-applets.txt,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-test.sh,dst=/smoke-test,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /busybox "${SOURCE_VERSION}" /expected-applets.txt
...
if [[ "${installed_sha256}" != "${candidate_sha256}" ]]; then
    echo "error: installed busybox does not match the validated candidate" >&2
    exit 1
fi
