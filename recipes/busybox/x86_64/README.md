# Static BusyBox 1.38.0 for x86-64

From the repository root, run:

```sh
./build.sh busybox x86_64
```

The command requires Bash, Docker, Docker Buildx, `file`, `readelf`, and
`sha256sum`. It consumes the immutable x86-64 builder lock and
replaces `artifacts/x86_64/busybox` only after complete validation.
...
The build uses the documented x86_64 compiler baseline, reconciles every
