...
if ! printf '%s\n' "${elf_header}" | grep -Eq \
    ...
    echo "error: busybox static PIE lacks an executable entry point or PT_LOAD" >&2
    exit 1
fi
if printf '%s\n' "${program_headers}" | grep -Eq '^[[:space:]]*INTERP[[:space:]]' ||
    printf '%s\n' "${dynamic_tags}" | grep -q '(NEEDED)'; then
    echo "error: busybox has a dynamic interpreter or shared dependency" >&2
    exit 1
fi
if ! printf '%s\n' "${dynamic_tags}" | grep -Eq '\(FLAGS_1\).*PIE([[:space:]]|$)' ||
    printf '%s\n' "${dynamic_tags}" | grep -Eq '\(TEXTREL\)|FLAGS.*TEXTREL'; then
    echo "error: busybox does not satisfy the static-PIE dynamic-tag contract" >&2
    exit 1
fi
if printf '%s\n' "${section_headers}" | grep -Eq '[.]debug|[.]symtab'; then
    echo "error: busybox retains debug or full symbol-table sections" >&2
    exit 1
fi
