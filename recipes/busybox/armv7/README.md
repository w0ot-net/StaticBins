# Static BusyBox 1.38.0 for ARMv7

From the repository root, run:

```sh
./build.sh busybox armv7
```

The command requires Bash, Docker, Docker Buildx, `file`, `readelf`, and
`sha256sum`. It consumes the immutable ARMv7 builder lock and
replaces `artifacts/armv7/busybox` only after complete validation.
...
The build uses the documented armv7 compiler baseline, reconciles every
