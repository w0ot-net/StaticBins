# Static netstat 2.10 for 32-bit x86

From the repository root, run:

```sh
./build.sh netstat x86
```

The command requires Bash, Docker, Docker Buildx, `file`, `readelf`, and
`sha256sum`. It consumes the immutable 32-bit x86 builder in
`builders/x86/environment.lock` and replaces `artifacts/x86/netstat`
...
relocations. Source, configuration, patch,
