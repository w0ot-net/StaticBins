# Static netstat 2.10 for x86-64

From the repository root, run:

```sh
./build.sh netstat x86_64
```

The command requires Bash, Docker, Docker Buildx, `file`, `readelf`, and
`sha256sum`. It consumes the immutable x86-64 builder in
`builders/x86_64/environment.lock` and replaces `artifacts/x86_64/netstat`
