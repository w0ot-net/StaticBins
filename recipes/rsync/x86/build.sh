...
readonly OUTPUT_FILE="${OUTPUT_DIR}/rsync"
...
if ! grep -Fq "[GNUPG:] VALIDSIG ${SOURCE_SIGNER_FINGERPRINT} " \
    ...
    echo "error: rsync signature did not match the locked fingerprint" >&2
    exit 1
fi
...
echo "Building rsync ${SOURCE_VERSION} for 32-bit x86 with Docker Buildx..."
docker buildx build \
    --platform "${PLATFORM}" \
...
candidate="${temporary_dir}/rsync"
"${SCRIPT_DIR}/validate-elf.sh" "${candidate}"
docker run --rm \
    --platform "${PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/rsync,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-test.sh,dst=/smoke-test,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /rsync "${SOURCE_VERSION}"
...
if [[ "${installed_sha256}" != "${candidate_sha256}" ]]; then
    echo "error: installed rsync does not match the validated candidate" >&2
    exit 1
fi
