...
if ! readelf -hW "${fixture_object}" | grep -Eq \
    ...
    echo "error: objdump smoke object has the wrong target identity" >&2
    exit 1
fi
docker run --rm \
    --platform "${TARGET_PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/objdump,readonly" \
    ...
    "${BUILDER_IMAGE}" \
    /smoke-test /objdump /fixture.o "${EXPECTED_VERSION_OUTPUT}"
