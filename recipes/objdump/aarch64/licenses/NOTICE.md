# GNU objdump 2.47 distribution notice

`artifacts/aarch64/objdump` is built from the exact signed GNU Binutils archive
recorded in `../source.lock` and retained under `../sources/`. The objdump,
BFD, opcodes, and SFrame code selected by this build is distributed under
...
Only objdump crosses the recipe's artifact boundary. Temporary sibling tools
