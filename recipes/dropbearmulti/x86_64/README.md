# Static Dropbear 2026.94 for x86-64

From the repository root, run:

```sh
./build.sh dropbearmulti x86_64
```
...
`builders/x86_64/environment.lock` and replaces
`artifacts/x86_64/dropbearmulti` only after complete validation.
...
```sh
./artifacts/x86_64/dropbearmulti ssh user@example.com
rsync -e './artifacts/x86_64/dropbearmulti dbclient' source/ host:dest/
```
