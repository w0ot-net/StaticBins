# Static Dropbear 2026.94 for ARMv7

From the repository root, run:

```sh
./build.sh dropbearmulti armv7
```

The command requires Bash, Docker, Docker Buildx, `file`, `gpgv`, `readelf`,
and `sha256sum`. It consumes the immutable ARMv7 builder in
`builders/armv7/environment.lock` and replaces
`artifacts/armv7/dropbearmulti` only after complete validation.
...
```sh
./artifacts/armv7/dropbearmulti ssh user@example.com
rsync -e './artifacts/armv7/dropbearmulti dbclient' source/ host:dest/
```
...
The output is a stripped ARMv7 ELF32 EABI5 hard-float static PIE with no interpreter, shared
