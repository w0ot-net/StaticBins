# Static Dropbear 2026.94 for AArch64

From the repository root, run:

```sh
./build.sh dropbearmulti aarch64
```

The command requires Bash, Docker, Docker Buildx, `file`, `gpgv`, `readelf`,
and `sha256sum`. It consumes the immutable AArch64 builder in
`builders/aarch64/environment.lock` and replaces
`artifacts/aarch64/dropbearmulti` only after complete validation.
...
```sh
./artifacts/aarch64/dropbearmulti ssh user@example.com
rsync -e './artifacts/aarch64/dropbearmulti dbclient' source/ host:dest/
```
