...
fixture_object="${temporary_dir}/fixture.o"
fixture_archive="${temporary_dir}/libfixture.a"
env TARGET_LOCK="${TARGET_LOCK}" "${SCRIPT_DIR}/validate-elf.sh" "${candidate}"
if ! readelf -hW "${fixture_object}" | grep -Eq \
    "Class:[[:space:]]+${EXPECTED_CLASS}" ||
    ! readelf -hW "${fixture_object}" | grep -Eq \
    "Machine:[[:space:]]+${EXPECTED_MACHINE}"; then
    echo "error: nm smoke object has the wrong target identity" >&2
    exit 1
fi
docker run --rm \
    --platform "${TARGET_PLATFORM}" \
    --network none \
    --mount "type=bind,src=${candidate},dst=/nm,readonly" \
    --mount "type=bind,src=${fixture_object},dst=/fixture.o,readonly" \
    --mount "type=bind,src=${fixture_archive},dst=/libfixture.a,readonly" \
    --mount "type=bind,src=${SCRIPT_DIR}/smoke-test.sh,dst=/smoke-test,readonly" \
    "${BUILDER_IMAGE}" \
    /smoke-test /nm /fixture.o /libfixture.a "${EXPECTED_VERSION_OUTPUT}"
