# GNU nm 2.47 distribution notice

`artifacts/aarch64/nm` is built from the exact signed GNU Binutils archive
recorded in `../source.lock` and retained under `../sources/`. The nm, BFD,
and SFrame code selected by this build is distributed under
...
Only nm crosses the recipe's artifact boundary. Temporary sibling tools
